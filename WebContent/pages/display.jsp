...
<body>
	...
	<div id="mainContent">
		...
		%><center>
		<br><br><h2>Sorry! No result</h2></center>
		<%
			} else {
		%>
		<center>
			<table width="75%" border=3>
				<tr>
					<th scope="col" width="25%"><font color="blue">name &
							Description</font></th>
					<th scope="col" width="25%"><font color="blue">Price($)</font></th>
					<th scope="col" width="25%"><font color="blue">Time </font></th>
					<th scope="col" width="10%"><font color="blue">Seller</font></th>
					<th scope="col" width="10%"><font color="blue">Highest
							Bidder</font></th>
				...
				<tr>
					<td><a
						...
						style="font-size: 25px"> <b><%=auctions.get(size - i).getName()%></b><br></a>
						<%=auctions.get(size - i)
										.getDescription()%></td>
					<td><b>Current Price: </b><font color="red"><%=auctions.get(size - i)
										.getCurrentPrice()%></font><br> <b>Start
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Price: </b><%=auctions.get(size - i)
										.getStartPrice()%><br> <b>Buy Out Price: </b><%=auctions.get(size - i).getBuyOut()%></td>
					<td><b>Finish&nbsp; Time: </b><br> <font color="red"><%=auctions.get(size - i)
										.getAuctionTime()%></font><br> <b>Start&nbsp;&nbsp; Time: </b><br><%=auctions.get(size - i).getStartTime()%><br>
						<%
							leftTime = auctions.get(size - i)
																		.getAuctionTime().getTime()
																		- System.currentTimeMillis();
						%> <b>Minutes Left: <br></b><%=MyUtils.getDay(leftTime)%> Days
						<%=MyUtils.getHour(leftTime)%> Hours <%=MyUtils.getMin(leftTime)%>
						minutes</td>
					<td><h2><%=auctions.get(size - i).getUserName()%></h2></td>
					<td><h2>
							<b><%=auctions.get(size - i).getWinner()%></b>
						</h2></td>
					<%
						if (auctions.get(size - i).getSold() == 0)
															message = "<font color=\"red\"><h2>Sold</h2></font>";
														else if (auctions.get(size - i).getSold() == 1)
															message = "<font color=\"blue\"><h2>Available</h2></font>";
														else
															message = "<h2>Failed</h2>";
					%>
					<td><%=message%></td>
				</tr>
				<%
				...
				%>
				<tr>
					<td><a href="bidAuction?auction=<%=auctions.get(i).getID()%>"
						style="font-size: 25px"> <b><%=auctions.get(i).getName()%></b><br></a>
					...
					<td><b>Current Price: </b><font color="red"><%=auctions.get(i).getCurrentPrice()%></font><br>
						...
						<b>Buy Out Price: </b><%=auctions.get(i).getBuyOut()%></td>
					<td><b>Finish&nbsp; Time: </b><br> <font color="red"><%=auctions.get(i).getAuctionTime()%></font><br>
						...
						<%
							leftTime = auctions.get(i).getAuctionTime()
																			.getTime()
																			- System.currentTimeMillis();
						%> <b>Minutes Left: <br></b><%=MyUtils.getDay(leftTime)%> Days
						<%=MyUtils.getHour(leftTime)%> Hours <%=MyUtils.getMin(leftTime)%>
					...
					<td><b>Finish&nbsp; Time: </b><br> <font color="red"><%=auctions.get(i).getAuctionTime()%></font><br>
						...
						<%
							leftTime = auctions.get(i).getAuctionTime()
																		.getTime()
																		- System.currentTimeMillis();
						%> <b>Minutes Left: <br></b><%=MyUtils.getDay(leftTime)%>
						Days <%=MyUtils.getHour(leftTime)%> Hours <%=MyUtils.getMin(leftTime)%>
						minutes</td>
					<td><h2><%=auctions.get(i).getUserName()%></h2></td>
					<td><h2>
							<b><%=auctions.get(i).getWinner()%></b>
